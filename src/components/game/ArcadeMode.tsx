...
export function ArcadeMode({ 
  ...
  // Map words from the new structure
  ...
  const spawnBubble = useCallback(() => {
    ...
    const speed = 0.2 + (level * 0.1); // Linear speed increase
    ...
    const newBubble: ActiveBubble = {
      ...
      hebrew: word.hebrew,
      x,
      y: -10, // Start just above the container
      speed,
      color
    };
  ...
  const updateGame = useCallback(() => {
    ...
    setBubbles(prev => {
      ...
      for (const b of prev) {
        ...
        // ONLY explode and deduct life if it hits the bottom (90% of container height)
        if (nextY > 90) { 
          heartsLost++;
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    // Find the bubble that matches the Hebrew translation typed by the user
    const targetIdx = bubbles.findIndex(b => b.hebrew === cleanInput || b.hebrew === userInput.trim());
        ...
        <div className="max-w-2xl w-full bg-white rounded-[48px] p-12 shadow-2xl border-t-8 border-orange-500 text-center">
          ...
          <div className="grid grid-cols-2 gap-4 mb-10">
            ...
            <div className="bg-indigo-50 p-6 rounded-[32px] border-2 border-indigo-100">
              ...
              <p className="text-xs font-bold text-indigo-400 uppercase tracking-widest">Max Level</p>
            </div>
          </div>

          <div className="text-left mb-10">
            <h3 className="font-bold text-slate-700 mb-4 border-b pb-2">Words missed:</h3>
            <div className="flex flex-wrap gap-2 max-h-40 overflow-y-auto p-2 bg-slate-50 rounded-2xl">
              ...
              )) : <p className="text-slate-400 italic">None! Perfect run.</p>}
            </div>
          </div>

          <div className="space-y-4">
            <Button onClick={() => { setGameState("ready"); setHearts(5); setScore(0); setCombo(0); setMissedWords([]); setLevel(1); }} className="w-full chunky-button chunky-primary py-8 text-xl">
              PLAY AGAIN
            </Button>
  ...
  return (
    <div className="min-h-screen bg-sky-400 relative overflow-hidden font-headline">
      ...
      {/* Game Stage - Linear drop physics */}
      <div className="w-full h-full absolute inset-0">
        {bubbles.map(b => (
          <div 
            key={b.id}
            className={cn(
              "absolute w-32 h-32 rounded-full border-4 border-white/40 shadow-xl flex flex-col items-center justify-center p-4 transition-all duration-300",
              b.color
            )}
            style={{ 
              left: `${b.x}%`, 
      ...
      <div className="absolute bottom-0 inset-x-0 p-12 flex justify-center bg-gradient-to-t from-sky-500 to-transparent">
        <form onSubmit={handleSubmit} className="w-full max-w-2xl relative">
          <Input 
            ...
            placeholder="Type Hebrew and press Enter..."
