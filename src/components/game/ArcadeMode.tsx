...
export function ArcadeMode({ 
  ...
  const spawnOneBubble = useCallback(() => {
    ...
    // Level 1: 16s duration. Progressively faster.
    const duration = Math.max(7, 16 - (level - 1) * 1.5);
  ...
  useEffect(() => {
    if (gameState === "playing" && spawnedCount < 10) {
      // Level 1: 3.5s interval. Progressively faster.
  ...
  if (gameState === "ready") {
    return (
      <div className="min-h-screen bg-gradient-to-br from-blue-100 via-indigo-100 to-purple-200 flex items-center justify-center p-6">
        <div className="max-w-xl w-full bg-white/95 backdrop-blur-md rounded-[40px] p-10 shadow-2xl border-none text-center">
          ...
          <div className="bg-slate-50 p-6 rounded-3xl mb-8 border border-slate-100 text-left shadow-inner">
            ...
            <div className="space-y-3">
              {vocabData.weeks.map(w => (
                <div 
                  key={w.week_id} 
                  className="flex items-center space-x-3 cursor-pointer p-2 hover:bg-white rounded-xl transition-all duration-300 hover:scale-105 active:scale-95 shadow-sm sm:shadow-none" 
                  onClick={() => toggleWeek(w.week_id)}
                >
                  <Checkbox checked={selectedWeeks.includes(w.week_id)} className="h-5 w-5 border-2" />
          ...
          <Button 
            ...
            onClick={initGame} 
            className="w-full chunky-button chunky-primary text-xl py-8 transition-all duration-300 hover:scale-105 hover:shadow-xl active:scale-95 cursor-pointer shadow-lg"
          >
            START GAME
          </Button>
  ...
  if (gameState === "gameover") {
    return (
      <div className="min-h-screen bg-gradient-to-br from-blue-100 via-indigo-100 to-purple-200 flex items-center justify-center p-6">
        <div className="max-w-2xl w-full bg-white/95 backdrop-blur-md rounded-[40px] p-12 shadow-2xl border-none text-center">
          <Trophy className="w-20 h-20 text-amber-400 mx-auto mb-6 transition-all duration-700 hover:scale-125 drop-shadow-md" />
          <h1 className="text-4xl font-headline font-bold text-slate-800 mb-8">Game Over!</h1>
          
          <div className="grid grid-cols-2 gap-4 mb-10">
            <div className="bg-sky-50 p-6 rounded-[32px] shadow-sm">
          ...
          <div className="text-left mb-10">
            ...
            <div className="flex flex-wrap gap-2 max-h-40 overflow-y-auto p-4 bg-slate-50 rounded-2xl border border-slate-100 shadow-inner">
          ...
          <div className="space-y-4">
            <Button onClick={() => setGameState("ready")} className="w-full chunky-button chunky-primary py-8 text-xl transition-all duration-300 hover:scale-105 hover:shadow-xl active:scale-95 cursor-pointer shadow-lg">
              TRY AGAIN
            </Button>
    ...
    <div className="fixed inset-0 bg-sky-400 overflow-hidden font-headline select-none">
      <div className="absolute top-0 inset-x-0 p-6 flex justify-between items-start z-30 pointer-events-none">
        <div className="flex items-center gap-4 pointer-events-auto">
          <Button variant="ghost" onClick={onBack} className="bg-white/30 hover:bg-white/50 backdrop-blur rounded-2xl text-white font-bold transition-all duration-300 hover:scale-110 active:scale-95 shadow-md border border-white/20">
            <ArrowLeft className="w-5 h-5 mr-2" /> EXIT
          </Button>
          <div className="bg-white/95 backdrop-blur px-6 py-2 rounded-2xl shadow-xl border-b-4 border-slate-200">
            <div className="flex items-center gap-2">
              <span className="text-slate-400 text-xs font-bold uppercase tracking-widest">Lv</span>
        ...
        <div className="flex flex-col items-end gap-2">
          <div className="bg-white/95 backdrop-blur px-6 py-3 rounded-[24px] shadow-xl border-b-4 border-slate-200 flex gap-2">
            {[...Array(5)].map((_, i) => (
          ...
          <div className="bg-amber-400 text-white px-8 py-2 rounded-2xl shadow-xl border-b-4 border-amber-600 font-bold text-2xl animate-pulse">
            {score}
          </div>
        </div>
      </div>
      ...
      <div className="absolute bottom-0 inset-x-0 p-8 flex justify-center bg-gradient-to-t from-sky-600/50 to-transparent z-20">
        <form onSubmit={handleSubmit} className="w-full max-w-2xl relative">
          ...
          />
          <Button type="submit" className="absolute right-3 top-3 bottom-3 rounded-[24px] chunky-primary px-10 text-xl transition-all duration-300 hover:scale-105 active:scale-95 cursor-pointer shadow-md">POP!</Button>
        </form>
      </div>
    </div>
  );
